...
/* ══════════════════════════════════════════════════
   CASCADING SELECTS - ACADEMIC
   ══════════════════════════════════════════════════ */

function initAcademicCascadingSelects() {
    const nivelSelect = document.getElementById('nivel');
    const modalidadSelect = document.getElementById('modalidad');
    const moduloSelect = document.getElementById('modulo');
    const horarioSelect = document.getElementById('horario');

    // ── Horarios by modalidad ──
    const horariosPorModalidad = {
        'Sabatinos': ['09h30-13h00', '14h00-17h30'],
        'Intensivos': ['07h00-09h00', '10h00-12h00', '15h00-17h00', '17h30-19h30', '18h30-20h30', '19h30-21h30'],
        'Super Intensivos': ['09h00-12h00', '14h00-17h00', '18h30-21h30'],
        'Juveniles': ['10h00-13h15'],
        'Vacacional': ['09h00-13h00'],
        'Preparación GZ': [] // No specific hours listed
    };

    // ── Update modules based on nivel and modalidad ──
    function updateModulos() {
        ...
        clearValidation('modulo');

        if (!nivel || !modalidad) {
            moduloSelect.innerHTML = '<option value="">Primero seleccione nivel y modalidad</option>';
            return;
        }

        let modulos = [];

        // Determine modules based on level and modalidad
        if ((nivel === 'A1' || nivel === 'A2') && (modalidad === 'Intensivos' || modalidad === 'Super Intensivos')) {
            modulos = ['Módulo 1', 'Módulo 2'];
        } else if ((nivel === 'B1' || nivel === 'B2') && (modalidad === 'Intensivos' || modalidad === 'Super Intensivos')) {
            modulos = ['Módulo 1', 'Módulo 2', 'Módulo 3'];
        } else if ((nivel === 'A1' || nivel === 'A2' || nivel === 'B1' || nivel === 'B2') && 
                   (modalidad === 'Sabatinos' || modalidad === 'Juveniles')) {
            modulos = ['Módulo 1', 'Módulo 2', 'Módulo 3', 'Módulo 4'];
        } else {
            // Default for other combinations (Preparación GZ, Vacacional)
            modulos = ['Módulo 1', 'Módulo 2', 'Módulo 3', 'Módulo 4'];
        }
        ...
        // Animation
        moduloSelect.style.animation = 'none';
        moduloSelect.offsetHeight;
        moduloSelect.style.animation = 'fadeInUp 0.3s ease-out';
    }

    // ── Update horarios based on modalidad ──
    function updateHorarios() {
        ...
        clearValidation('horario');

        if (!modalidad) {
            horarioSelect.innerHTML = '<option value="">Primero seleccione una modalidad</option>';
            return;
        }

        const horarios = horariosPorModalidad[modalidad] || [];

        if (horarios.length === 0) {
            horarioSelect.innerHTML = '<option value="Por definir">Por definir</option>';
        ...
        // Animation
        horarioSelect.style.animation = 'none';
        horarioSelect.offsetHeight;
        horarioSelect.style.animation = 'fadeInUp 0.3s ease-out';
    }

    // ── Event listeners ──
    nivelSelect.addEventListener('change', updateModulos);
...
const validationRules = {
    ...
    tipoAsistencia: {
        validate: (val) => val !== '',
        message: 'Seleccione un tipo de asistencia'
    },
    horario: {
        validate: (val) => val !== '',
...
function closeModal() {
    ...
    // Reset academic selects
    document.getElementById('modulo').innerHTML = '<option value="">Primero seleccione nivel y modalidad</option>';
    document.getElementById('modulo').disabled = true;
    document.getElementById('horario').innerHTML = '<option value="">Primero seleccione una modalidad</option>';
